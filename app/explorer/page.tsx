...
import { heatmapInsight, sankeyInsight } from "@/lib/data/insights";
...
export default function ExplorerPage() {
  const { current, comparison, completeness, channel, setChannel } = useWorkspace();
  const { push } = useToast();
  ...
  return (
    <PageShell>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 rounded-lg border border-line bg-surface p-4 shadow-xs md:grid-cols-3 xl:grid-cols-5">
        {[
          { k: "Revenue", v: money(current.totalRevenue), cur: current.totalRevenue, prev: comparison.totalRevenue, better: true },
          ...
          { k: "CAC", v: money(current.cac), cur: current.cac, prev: comparison.cac, better: false },
          { k: "Contribution margin", v: `${money(current.contributionPerOrder)} /order`, cur: current.contributionPerOrder, prev: comparison.contributionPerOrder, better: true },
          { k: "Net profit", v: money(current.netProfit), cur: current.netProfit, prev: comparison.netProfit, better: true },
        ].map((s) => (
        ...
        <div className="min-w-0 space-y-3">
          ...
          <Card>
            <CardHeader
              ...
              action={<Chip tone="neutral">{num(skuRows.length - 1)} SKUs</Chip>}
            />
            <div className="mt-4">
              <MarginHeatmap skus={skuRows} />
            </div>
            <ChartInsight insight={heatmapInsight(skuRows.filter((s) => !s.isLongTail))} className="mt-4" />
          </Card>
        </div>
